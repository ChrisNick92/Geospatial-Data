...
class cropped_dataset(Dataset):
    def __init__(self, path, img_name, crop_size, normalization:List[TransformFun] = [],
        ...
        self.up_right_corners = [] # the up-right corners of the cropped images

        self.find_up_right_corners()

    def find_up_right_corners(self):
        for row in range(0, self.max_height, self.crop_size):
            for col in range(0, self.max_width, self.crop_size):
                self.up_right_corners.append((row,col))

    def __getitem__(self, idx):
        i,j = self.up_right_corners[idx]
    ...
    def __len__(self):
        return len(self.up_right_corners)
